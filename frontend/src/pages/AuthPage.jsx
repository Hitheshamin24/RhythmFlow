            ...
            <div className="relative group">
              ...
              >
                {show ? (
                  ...
                  <span className="text-[10px] font-bold uppercase">Show</span>
                )}
              </button>
            </div>
          </div>
      ...
      {showForgotModal && (
        <ModalOverlay onClose={() => setShowForgotModal(false)}>
          ...
          <form onSubmit={handleForgotSubmit} className="space-y-6">
            ...
            {fpError && (
              <p className="text-center text-xs text-rose-600 bg-rose-50 py-2 rounded-lg font-medium">
                {fpError}
              </p>
            )}
            {fpMessage && (
              <p className="text-center text-xs text-emerald-600 bg-emerald-50 py-2 rounded-lg font-medium">
                {fpMessage}
              </p>
            )}
            ...
            <div className="flex gap-3">
              ...
              <button
                type="submit"
                disabled={fpLoading}
                className="flex-1 py-2.5 rounded-xl bg-rose-600 text-white text-xs font-bold hover:bg-rose-700 transition-colors flex items-center justify-center gap-2"
              >
...
const ModalOverlay = ({ children, onClose }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1F1216]/60 backdrop-blur-sm px-4 animate-in fade-in duration-200">
    <div className="bg-white rounded-[20px] w-full max-w-sm shadow-2xl border border-white/40 p-6 md:p-8 relative animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
      <button
        onClick={onClose}
        className="absolute right-4 top-4 text-slate-400 hover:text-slate-700 bg-slate-50 hover:bg-slate-100 p-1.5 rounded-full transition-all"
      >
        <X size={16} />
      </button>
      {children}
